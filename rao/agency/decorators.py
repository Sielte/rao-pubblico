...
from agency.utils.utils import is_admin
    ...
    def decorator(function):
        def onCall(request, *args, **kwargs):
            try:
                ...
                LOG.error("Errore in decorator login_required: {}".format(str(e)))
                return HttpResponseRedirect(reverse('agency:logout_agency'))
        ...
        def onCall(request, *args, **kwargs):
            try:
                ...
                if is_admin(params['username']):
                    ...
                    LOG.error("Errore in decorator admin_required non sei utente Admin")
                    return HttpResponseRedirect(reverse('agency:list_identity', kwargs={'t', token}))
            except Exception as e:
                LOG.error("Errore in decorator admin_required:{}".format(str(e)))
                return redirect(settings.LOGIN_URL)
        ...
        def onCall(request, *args, **kwargs):
            try:
                ...
                if not is_admin(params['username']):
                    ...
                    LOG.error("Errore in decorator operator_required non sei utente Operator")
                    return HttpResponseRedirect(reverse('agency:list_identity', kwargs={'t', token}))
            except Exception as e:
                LOG.error("Errore in decorator operator_required: {}".format(str(e)))
                return redirect(settings.LOGIN_URL)
        ...
        def onCall(request, *args, **kwargs):
            try:
                if check_db_not_altered():
                    ...
                    LOG.error("Errore in decorator only_one_admin il db è stato alterato.")
                    params = {
                ...
                LOG.error("Errore in decorator only_one_admin: {}".format(str(e)))
                return redirect(settings.LOGIN_URL)
